...
ConVar g_cvMaxNPCsClient;
ConVar g_cvMaxVehiclesClient;
ConVar g_cvRemoveDisc;
ConVar g_cvFakeZom;
ConVar g_cvProtectMap;
...
public int OnPluginStart()
{
	...
	BuildPath(Path_SM, g_sClientPrefsPath, 64, "data/celmod/clientprefs.txt");
}
...
int changeBeam(int client, int Ent)
{
	...
	if (TR_DidHit(TraceRay))
	{
		...
		EmitSoundToAll(g_sToolSound, Ent, 2, 100, 0, 1, 100, -1, NULL_VECTOR, NULL_VECTOR, true, 0);
	}
...
int ReadQue(int client)
{
	...
	while (I < 1000)
	{
		if (StrEqual(g_sUndoQue[client][0][0][I], "", false))
		{
			return I;
		}
		I++;
...
int WriteQue(int client, int ent, int num)
{
	...
	GetEntPropString(ent, Prop_Data, "m_ModelName", queString[4], 128);
	GetEntPropString(ent, Prop_Data, "m_iGlobalname", queString[68], 128);
	skinNum = GetEntProp(ent, Prop_Data, "m_nSkin", 1);
	solid = GetEntProp(ent, Prop_Send, "m_nSolidType", 1);
	...
	renderFx = GetEntProp(ent, Prop_Send, "m_nRenderFX", 1);
	GetEntPropVector(ent, Prop_Data, "m_vecAbsOrigin", entOrgn);
	GetEntPropVector(ent, Prop_Data, "m_angRotation", angRot);
	entFlags = GetEntProp(ent, Prop_Data, "m_spawnflags", 1);
	takedamage = GetEntProp(ent, Prop_Data, "m_takedamage", 1);
	...
	ImplodeStrings(queString, 18, "*", g_sUndoQue[client][0][0][num], 255);
}
...
int FindOwner(int client, int Ent)
{
	if (IsValidEntity(Ent))
	{
		...
		GetEntPropString(Ent, Prop_Data, "m_iGlobalname", entGlobal, 64);
...
int CountAllNPCs()
{
	...
	while (AllE < MaxEnts)
	{
		...
		{
			char gName[256];
			GetEntPropString(AllE, Prop_Data, "m_iGlobalname", gName, 255);
...
public int OnMapStart()
{
	...
	g_iEntDissolve = CreateEntityByName("env_entity_dissolver", -1);
	DispatchKeyValue(g_iEntDissolve, "target", "deleted");
	DispatchKeyValue(g_iEntDissolve, "magnitude", "50");
	DispatchKeyValue(g_iEntDissolve, "dissolvetype", "3");
	DispatchSpawn(g_iEntDissolve);
	DispatchKeyValue(g_iEntDissolve, "classname", "cel_entity_dissolver");
	int metro = CreateEntityByName("npc_metropolice", -1);
	DispatchSpawn(metro);
	CreateTimer(0.2, RemoveCop, metro, 0);
	g_iEntIgnite = CreateEntityByName("env_entity_igniter", -1);
	DispatchKeyValue(g_iEntIgnite, "target", "ignited");
	DispatchSpawn(g_iEntIgnite);
	DispatchKeyValue(g_iEntIgnite, "classname", "cel_entity_igniter");
	if (GetConVarBool(g_cvProtectMap))
	{
		int MaxEnts;
		int E;
...
{
	RemoveEdict(g_iEntDissolve);
	RemoveEdict(g_iEntIgnite);
}
...
public int OnClientDisconnect(int client)
{
	...
	int E;
	Me = GetMaxEntities();
	if (GetConVarBool(g_cvRemoveDisc))
	{
		E = 1;
		while (E < Me)
		{
			if (IsValidEntity(E))
			{
				if (FindOwner(client, E) == 1)
				{
					...
					if (StrContains(pClass, "prop_vehicle", false))
					{
						...
						{
							AcceptEntityInput(GetEntPropEnt(E, Prop_Data, "m_hMoveChild"), "turnoff", -1, -1, 0);
						}
						if (StrEqual(pClass, "cel_music", false))
						{
							char mBreak[12][128];
							ExplodeString(g_sEntMusicPath[E][0][0], "|", mBreak, 3, 128);
							StopSound(E, 0, mBreak[0][mBreak]);
						}
					}
					else
					{
						if (GetEntPropEnt(E, Prop_Data, "m_hPlayer") != -1)
						{
...
public Action Command_spawnprop(int client, int Args)
{
	...
	if (SpawnpropTime[client][0][0] <= GetGameTime() - 1)
	{
		...
		FileToKeyValues(Props, g_sPropPath);
		...
		if (!StrEqual(propBuffer[0][propBuffer], "3", false))
		{
			if (GetEntProp(propEnt, Prop_Data, "m_takedamage", 4) == 2)
			{
			...
			if (StrEqual(propBool, "god", false))
			{
				if (GetEntProp(propEnt, Prop_Data, "m_takedamage", 4) == 2)
				{
					SetEntProp(propEnt, Prop_Data, "m_takedamage", any0, 1);
				}
			}
		}
		else
...
public Action Command_previewprop(int client, int Args)
{
	...
	DispatchSpawn(propEnt);
	SetEntPropEnt(propEnt, Prop_Data, "m_hMoveParent", client);
	SetEntProp(propEnt, Prop_Send, "m_nSolidType", any0, 4);
	SetEntProp(propEnt, Prop_Data, "m_takedamage", any0, 4);
...
public Action Command_pasteprop(int client, int Args)
{
	...
	if (PasteTime[client][0][0] <= GetGameTime() - 1)
	{
		...
		{
			if (StrEqual(g_sCPClass[client][0][0], "prop_physics_m_breakable", false))
			{
				cpEnt = CreateEntityByName("prop_physics_multiplayer", -1);
			}
			cpEnt = CreateEntityByName(g_sCPClass[client][0][0], -1);
		}
		if (GetConVarInt(g_cvMaxPropsClient) <= CountProps(client))
		{
			Format(g_sTempString, 255, "You've reached your max prop count(%d).", GetConVarInt(g_cvMaxPropsClient));
		...
		DispatchKeyValue(cpEnt, "model", g_sCPModel[client][0][0]);
		...
		{
			AcceptEntityInput(cpEnt, "Kill", -1, -1, 0);
			if (StrEqual(g_sCPClass[client][0][0], "prop_physics", false))
			{
		...
		if (g_bCPBreakable[client][0][0])
		{
			SetEntProp(cpEnt, Prop_Data, "m_takedamage", any2, 1);
		}
		else
		{
			SetEntProp(cpEnt, Prop_Data, "m_takedamage", any0, 1);
		}
		if (g_bCPFrozen[client][0][0])
		{
...
public Action Command_msgs(int client, int Args)
{
	...
	FileToKeyValues(CPrefs, g_sClientPrefsPath);
	char steamID[256];
	char toggle[4];
	...
	KeyValuesToFile(CPrefs, g_sClientPrefsPath);
	CloseHandle(CPrefs);
	return Plugin_Handled;
}
public Action Command_advisor(int client, int Args)
{
	...
	SetEntProp(advisor, Prop_Data, "m_takedamage", any0, 1);
...
public Action Command_npccreate(int client, int Args)
{
	...
	if (GetConVarInt(g_cvNPCCvar) > CountAllNPCs())
	{
		if (GetConVarInt(g_cvMaxNPCsClient) > CountNPCs(client))
		{
			char npcA[32];
			char npcBuffer[8][64];
			char npcClass[64];
		...
		{
			Format(g_sTempString, 255, "You've reached your max NPC count(%d).", GetConVarInt(g_cvMaxNPCsClient));
			cmMsg(client, g_sTempString);
		}
	}
	else
...
public Action Command_pod(int client, int Args)
{
	...
	GetEntPropString(Ent, Prop_Data, "m_ModelName", podmodel, 128);
...
public Action Command_undoRemove(int client, int args)
{
	...
	else
	{
		...
		ExplodeString(g_sUndoQue[client][0][0][I], "*", undoString, 18, 255);
		int undoEnt;
		float entOrgn[3];
		float entRot[3];
		...
		SetEntProp(undoEnt, Prop_Data, "m_nSkin", StringToInt(undoString[8], 10), 1);
		SetEntProp(undoEnt, Prop_Send, "m_nSolidType", StringToInt(undoString[12], 10), 1);
		...
		SetEntProp(undoEnt, Prop_Data, "m_takedamage", StringToInt(undoString[40], 10), 4);
...
public Action Command_remove(int client, int args)
{
			...
			while (E < Me)
			{
				if (FindOwner(client, E) == 1)
				{
					...
					if (StrContains(pClass, "prop_vehicle", false))
					{
						...
						{
							char aBreak[12][128];
							ExplodeString(g_sEntMusicPath[E][0][0], "|", aBreak, 3, 128);
	...
	{
		GetEntPropString(Ent2, Prop_Data, "m_iName", tName, 32);
		if (!StrEqual(tName, "deleted", false))
		{
			GetEdictClassname(Ent2, classname, 255);
			int var2;
			if (StrEqual(classname, g_sPlayerTag, false))
			{
			...
			{
				airboatEnt = GetEntPropEnt(Ent2, Prop_Data, "m_hPlayer");
				if (airboatEnt != -1)
				{
			...
			{
				AcceptEntityInput(GetEntPropEnt(Ent2, Prop_Data, "m_hMoveChild"), "turnoff", -1, -1, 0);
			}
			else
			{
			...
			int randomDis;
			GetClientAbsOrigin(client, clientOrigin);
			GetEntPropVector(Ent2, Prop_Data, "m_vecAbsOrigin", EntOrigin);
			...
			{
				AcceptEntityInput(g_iEntDissolve, "dissolve", -1, -1, 0);
			}
			else
			{
...
public Action Command_unfreeze(int client, int args)
{
	...
	if (FindOwner(client, Ent2) != -1)
	{
		...
		if (StrContains(classname, g_sNPCTag, false))
		{
			int var2;
			if (StrEqual(classname, "prop_door_rotating", false))
			{
...
public Action Command_airboat(int client, int args)
{
	if (GetConVarInt(g_cvMaxVehiclesClient) > CountVehicles(client))
	{
		...
		SetEntProp(client, Prop_Data, "m_nImpulse", any83, 1);
...
public Action FindBoat(Handle timer, any client)
{
	...
	if (airEnt != -1)
	{
		...
		float airOrgn[3];
		GetEntPropVector(airEnt, Prop_Data, "m_vecAbsOrigin", airOrgn);
		airOrgn[8] += 10;
...
public Action Command_skin(int client, int args)
{
	...
	if (FindOwner(client, sEnt) != -1)
	{
		...
		int var1;
		if (!StrEqual(sClass, g_sPlayerTag, false))
		{
...
public Action Command_scene(int client, int args)
{
	...
	if (FindOwner(client, scEnt) != -1)
	{
		...
		if (StrContains(scClassname, g_sNPCTag, false))
		{
			if (StrContains(scClassname, g_sNPCTag, false))
			{
...
public Action Command_relationship(int client, int args)
{
	...
	if (FindOwner(client, rEnt) != -1)
	{
		...
		int var1;
		if (StrContains(rClassname, g_sNPCTag, false))
		{
			...
			{
				Format(g_sTempString, 255, "Set the relationship of %s to %s.", rname[4], rorient);
				cmMsg(client, g_sTempString);
			}
		}
		cmMsg(client, "This can only be done to NPCs.");
...
public Action Command_ignite(int client, int args)
{
	...
	if (FindOwner(client, bEnt) != -1)
	{
		...
		{
			if (StrEqual(bClassname, g_sPlayerTag, false))
			{
				cmMsg(client, "This entity cannot be ignited.");
				return Plugin_Handled;
			}
			if (StrContains(bClassname, g_sNPCTag, false))
			{
				char targetname[128];
				GetEntPropString(bEnt, Prop_Data, "m_iName", targetname, 128);
				DispatchKeyValue(bEnt, "targetname", "ignited");
				DispatchKeyValue(g_iEntIgnite, "lifetime", bSeconds);
				AcceptEntityInput(g_iEntIgnite, "ignite", -1, -1, 0);
			...
			if (!blockMsgs[client][0][0])
			{
				...
				if (StrEqual(brokeClass[0][brokeClass], "combine", false))
				{
					Format(g_sTempString, 255, "Ignited %s %s for %s seconds.", brokeClass[0][brokeClass], brokeClass[4], bSeconds);
					cmMsg(client, g_sTempString);
				}
				Format(g_sTempString, 255, "Ignited %s %s for %s seconds.", brokeClass[4], brokeClass[0][brokeClass], bSeconds);
				cmMsg(client, g_sTempString);
			}
			return Plugin_Handled;
		}
...
public Action Command_god(int client, int args)
{
	...
	if (FindOwner(client, gEnt) != -1)
	{
		...
		int var1;
		if (!StrEqual(gClassname, g_sPlayerTag, false))
		{
			if (StrEqual(gInput, "on", false))
			{
				int var2;
				if (GetEntProp(gEnt, Prop_Data, "m_takedamage", 4) == 2)
				{
					SetEntProp(gEnt, Prop_Data, "m_takedamage", any0, 1);
			...
			if (StrEqual(gInput, "off", false))
			{
				...
				{
					SetEntProp(gEnt, Prop_Data, "m_takedamage", any2, 1);
...
public Action Command_color(int client, int args)
{
	...
	if (FindOwner(client, cEnt) != -1)
	{
		...
		GetCmdArg(2, sGrn, 6);
		GetCmdArg(3, sBlu, 6);
		if (StrEqual(cClass, g_sPlayerTag, false))
		{
		...
		if (StrEqual(cClass, "cel_light", false))
		{
			int moveChild = GetEntPropEnt(cEnt, Prop_Data, "m_hMoveChild");
		...
		else
		{
			if (StrEqual(cClass, "cel_light", false))
			{
				...
				char lightName[32];
				char clightNum[8][32];
				GetEntPropString(moveChild, Prop_Data, "m_iName", lightName, 32);
				ExplodeString(lightName, "_", clightNum, 2, 32);
				Format(g_sTempString, 255, "Applied color to light %s.", clightNum[4]);
				cmMsg(client, g_sTempString);
			}
...
public Action Command_whoowns(int client, int args)
{
	...
	GetEntPropString(vEnt, Prop_Data, "m_iGlobalname", whos, 64);
	...
	if (EntOut)
	{
		Format(g_sTempString, 255, "Player \x04%s\x01 owns this entity.", clientname);
		cmMsg(client, g_sTempString);
	}
	else
	{
		...
		if (!StrEqual(whos, "", false))
		{
			...
			Format(g_sTempString, 255, "This entity was owned by %s\x04(\x01%s\x04)\x01.", ownerName[4], ownerName[0][ownerName]);
			cmMsg(client, g_sTempString);
		}
...
public Action Command_startMove(int client, int args)
{
	...
	else
	{
		...
		if (FindOwner(client, moveEnt) != -1)
		{
			...
			int var1;
			if (StrEqual(moveClass, g_sPlayerTag, false))
			{
...
public Action Command_solidity(int client, int args)
{
	...
	if (FindOwner(client, sEnt) != -1)
	{
		...
		int var1;
		if (!StrEqual(sClassname, g_sPlayerTag, false))
		{
...
public Action Command_sound(int client, int args)
{
	...
	if (GetConVarInt(g_cvMaxCelsClient) > CountCels(client))
	{
		if (SoundTime[client][0][0] <= GetGameTime() - 1)
		{
			...
			FileToKeyValues(Sounds, g_sSoundPath);
...
public Action Command_startCopy(int client, int Args)
{
	...
	if (!StrContains(cpClass, "prop_physics", false))
	{
		...
		if (GetEntProp(newEnt, Prop_Data, "m_takedamage", 4) == 2)
		{
			if (GetConVarInt(g_cvMaxBreakablesClient) <= CountBreakables(client))
			{
				Format(g_sTempString, 255, "You've reached your max breakable prop count(%d).", GetConVarInt(g_cvMaxBreakablesClient));
...
public Action Command_alpha(int client, int args)
{
	...
	if (FindOwner(client, amtEnt) != -1)
	{
		...
		if (!StrEqual(amtClass, "player", false))
		{
			...
			int blue;
			GetCmdArg(1, amt, 32);
			SetEntProp(amtEnt, Prop_Data, "m_nRenderMode", any1, 1);
...
public Action Command_rotate(int client, int args)
{
	...
	if (FindOwner(client, rotEnt) != -1)
	{
		...
		if (!StrEqual(rotClass, "player", false))
		{
			...
			{
				float rotAng[3];
				GetEntPropVector(rotEnt, Prop_Data, "m_angRotation", rotAng);
			...
			if (StrEqual(netClass, "CBasePropDoor", false))
			{
				...
				char doorskin[16];
				int doorColor[4];
				GetEntPropString(rotEnt, Prop_Data, "m_ModelName", mName, 128);
				int doorSkin = GetEntProp(rotEnt, Prop_Data, "m_nSkin", 4);
				int doorHardware = GetEntProp(rotEnt, Prop_Data, "m_nHardwareType", 4);
				...
				DispatchKeyValue(dEnt, "OnFullyOpen", "!caller,close,,3,-1");
				DispatchSpawn(dEnt);
				SetEntProp(dEnt, Prop_Data, "m_nRenderMode", any1, 1);
				SetEntityRenderColor(dEnt, doorColor[0], doorColor[4], doorColor[8], doorColor[12]);
				float doorOrgn[3];
				GetEntPropVector(rotEnt, Prop_Data, "m_vecAbsOrigin", doorOrgn);
				RemoveEdict(rotEnt);
...
public Action Command_autoStack(int client, int args)
{
	...
	else
	{
		...
		int eColor[4];
		int takedamage;
		GetEntPropString(stackEnt, Prop_Data, "m_ModelName", modelName, 128);
		...
		renderFx = GetEntProp(stackEnt, Prop_Send, "m_nRenderFX", 1);
		GetEntPropVector(stackEnt, Prop_Data, "m_vecAbsOrigin", entOrgn);
		GetEntPropVector(stackEnt, Prop_Data, "m_angRotation", angRot);
		skinNum = GetEntProp(stackEnt, Prop_Data, "m_nSkin", 1);
		entFlags = GetEntProp(stackEnt, Prop_Data, "m_spawnflags", 1);
		takedamage = GetEntProp(stackEnt, Prop_Data, "m_takedamage", 1);
		char SrenderFx[32];
		char SskinNum[32];
		char SentFlags[32];
		IntToString(renderFx, SrenderFx, 32);
		...
		int var2;
		if (GetEntProp(stackEnt, Prop_Data, "m_takedamage", 4) == 2)
		{
		...
		while (copies <= 3000)
		{
			...
			SetEntProp(copies, Prop_Data, "m_takedamage", takedamage, 1);
			...
			if (GetEntProp(copies, Prop_Data, "m_takedamage", 4) == 2)
			{
				if (StrEqual(sClass, "prop_physics", false))
				{
...
{
	int playerEnt = GetEntPropEnt(client, Prop_Data, "m_hVehicle");
	int var1;
	if (playerEnt != -1)
	{
